...
async function handleEvent(event) {
  ...
  if (userText === "คำคมฮีลใจ") {
    try {
      // แก้ไขปัญหาคำคมโดนตัดจบ: ปรับ Prompt ให้ส่งเฉพาะเนื้อหา และเพิ่มพื้นที่ Token
      const response = await ai.models.generateContent({
        model: 'gemini-2.5-flash',
        contents: 'จงแต่งคำคมฮีลใจ ให้พลังบวก 1 ข้อความ ความยาวประมาณ 2-3 ประโยค เน้นความสดใส ร่าเริง และใส่อีโมจิ เงื่อนไขสำคัญที่สุด: ห้ามมีคำเกริ่นนำ คำตอบรับ หรือคำอธิบายใดๆ ทั้งสิ้น (เช่น ห้ามพิมพ์ว่า แน่นอนค่ะ, นี่คือคำคมของคุณ, จัดไป) ให้ตอบกลับเฉพาะเนื้อหาของคำคมเพียวๆ เท่านั้น',
        config: {
          maxOutputTokens: 300 // ขยายพื้นที่ให้รองรับภาษาไทยและอีโมจิได้จนจบประโยค ไม่โดนหักดิบ
        }
      });
