...
async function handleEvent(event) {
  ...
  if (userText === "คำคมฮีลใจ") {
    try {
      // ปรับโครงสร้างพารามิเตอร์ให้ตรงตามสเปกของ SDK เวอร์ชันใหม่
      const response = await ai.models.generateContent({
        model: 'gemini-2.5-flash',
        contents: 'จงแต่งคำคมฮีลใจ ให้พลังบวก 1 ข้อความ ความยาวสั้นๆ แค่ 2-3 ประโยค เน้นความสดใส ร่าเริง และใส่อีโมจิเยอะๆ เงื่อนไขสำคัญที่สุด: ห้ามมีคำเกริ่นนำ คำตอบรับ หรือคำอธิบายใดๆ ทั้งสิ้น (เช่น ห้ามพิมพ์ว่า แน่นอนค่ะ, นี่คือคำคมของคุณ) ให้ตอบกลับเฉพาะเนื้อหาของคำคมเพียวๆ เท่านั้น',
        config: {
          maxOutputTokens: 400 // ย้ายมาอยู่ใน config ที่ถูกต้อง และขยายเป็น 400 เพื่อให้ภาษาไทยยาวแค่ไหนก็ไม่ขาด
        }
      });
